...
mod tests {
    ...
    fn executable(bundle: &std::path::Path) -> anyhow::Result<std::path::PathBuf> {
        let executable = bundle.join("Contents/MacOS/nexus-vpn");
    ...
    fn canonical_allowed_roots_and_nested_bundles_are_accepted() -> anyhow::Result<()> {
        ...
        let system_exe = executable(&system.join("Tools/Nexus VPN.app"))?;
        let user_exe = executable(&user.join("Network/Nexus VPN.app"))?;
    ...
    fn failed_staged_activation_preserves_existing_application() -> anyhow::Result<()> {
        ...
        let destination = root.join("Nexus VPN.app");
        let staging = root.join(".Nexus VPN.app.installing");
        let backup = root.join(".Nexus VPN.app.backup");
    ...
    fn canonicalization_accepts_symlink_in_and_rejects_symlink_escape() -> anyhow::Result<()> {
        ...
        let allowed_bundle = system.join("Nexus VPN.app");
        let allowed_exe = executable(&allowed_bundle)?;
        let link_in = downloads.join("Nexus VPN.app");
        ...
        assert!(matches!(
            evaluate_install_location_with_roots(&link_in.join("Contents/MacOS/nexus-vpn"), &home, &system),
            LaunchLocation::Allowed { .. }
        ));
        assert!(matches!(
            evaluate_install_location_with_roots(&link_out.join("Contents/MacOS/nexus-vpn"), &home, &system),
            LaunchLocation::Movable { .. }
        ));
    ...
    fn allowed_root_symlink_escape_is_not_authorized() -> anyhow::Result<()> {
        ...
        let escaped_exe = executable(&home.join("Applications/Nexus VPN.app"))?;
    ...
    fn translocation_and_missing_bundle_are_rejected_before_side_effects() {
        let translocated =
            std::path::Path::new("/private/var/folders/AppTranslocation/Nexus VPN.app/Contents/MacOS/nexus-vpn");
        assert_eq!(
        ...
        assert!(matches!(
            evaluate_install_location_with_roots(
                std::path::Path::new("/Users/test/Downloads/nexus-vpn"),
                std::path::Path::new("/Users/test"),
                std::path::Path::new("/Applications")
            ),
